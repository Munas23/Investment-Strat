...
class TurtleStrategy:
    ...
    def create_default_config(self, config_path):
        ...
        default_config = {
            ...
            # Original Turtle Risk Management Parameters
            "turtle_risk_management": True,
            "risk_per_unit": 0.01,              # 1% risk per unit (N-based sizing)
            "stop_loss_n": 2.0,                 # 2N stop loss rule
            "unit_addition_n": 0.5,             # Add units every 1/2 N intervals
            "n_period": 20,                     # 20-day period for N calculation
            
            # Position Limits (Original Turtle Rules)
            "max_units_single_market": 4,       # Max 4 units per individual market
            "max_units_closely_correlated": 6,  # Max 6 units in closely correlated markets
            "max_units_loosely_correlated": 10, # Max 10 units in loosely correlated markets  
            "max_units_single_direction": 12,   # Max 12 total units long OR short
            
            # Risk Scaling Rules
            "account_reduction_threshold": 0.10, # Reduce account size by 20% after 10% loss
            "account_reduction_amount": 0.20     # Amount to reduce account size
        },
        "backtest": {
