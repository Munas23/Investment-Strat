"""
Mark Minervini's Complete Championship Strategy - Enhanced with Pyfolio Analysis
=============================================================================

This version includes comprehensive portfolio analysis using pyfolio-reloaded
for professional-grade performance reporting and visualization.
"""
...
class MinerviniEnhancedStrategy(Strategy):
    """
    Enhanced Minervini Strategy with Pyfolio Integration
    
    Combines:
    1. Fundamental screening + technical timing + risk management
    2. Comprehensive pyfolio analysis and reporting
    3. Professional-grade tear sheets and metrics
    """
    ...
    def initialize(self):
        ...
        self.log_message("=== MINERVINI ENHANCED STRATEGY WITH PYFOLIO ===")
    ...
    def calculate_position_size(self, symbol, price, conviction_level):
        ...
        try:
            ...
            base_position_pct = {
                1: 0.10,  # 20% - minimal conviction
                2: 0.125,  # 25% - low conviction  
                3: 0.15,  # 30% - standard conviction
                4: 0.175,  # 35% - high conviction
                5: 0.20   # 40% - maximum conviction
    ...
    def on_strategy_end(self):
        ...
        self.log_message("=== MINERVINI ENHANCED STRATEGY COMPLETED ===")
        
        # Save basic CSV
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        if self.trades_log:
            trades_df = pd.DataFrame(self.trades_log)
            csv_filename = f"minervini_enhanced_trades_{timestamp}.csv"
        ...
        self.log_message(f"Fundamental Leaders Found: {len(self.fundamental_leaders)}")
        
        if PYFOLIO_AVAILABLE:
            self.log_message("✓ Pyfolio tear sheet generated (see HTML file)")
    ...
    def generate_pyfolio_analysis(self, timestamp):
        ...
        try:
            ...
            plt.savefig(f"{output_dir}/returns_tearsheet_{timestamp}.png", 
                       dpi=300, bbox_inches='tight')
            plt.close('all')
            self.log_message(f"✓ Returns tear sheet saved: returns_tearsheet_{timestamp}.png")
            ...
            plt.savefig(f"{output_dir}/simple_tearsheet_{timestamp}.png", 
                       dpi=300, bbox_inches='tight')
            plt.close('all')
            self.log_message(f"✓ Simple tear sheet saved: simple_tearsheet_{timestamp}.png")
            
            # 3. Generate and save key metrics to text file
            metrics_filename = f"{output_dir}/performance_metrics_{timestamp}.txt"
            ...
            self.log_message(f"  Benchmark: SPY")
    ...
    def save_performance_metrics(self, returns, benchmark, filename):
        ...
        try:
            with open(filename, 'w') as f:
                f.write("MINERVINI STRATEGY PERFORMANCE METRICS\n")
                f.write("=" * 50 + "\n\n")
    ...
    def create_basic_performance_report(self, returns, benchmark, timestamp):
        ...
        try:
            ...
            axes[0,0].plot(cum_returns.index, cum_returns, label='Strategy', linewidth=2)
            ...
            # Save plot
            plot_filename = f"pyfolio_reports/minervini_performance_{timestamp}.png"
...
def run_enhanced_minervini_backtest():
    """Run enhanced Minervini strategy with Pyfolio analysis"""
    try:
        print("=" * 80)
        print("MARK MINERVINI'S ENHANCED STRATEGY WITH PYFOLIO ANALYSIS")
        print("=" * 80)
        print("Features:")
        print("✓ Fundamental screening + Technical timing")
        print("✓ Championship risk management")
        if PYFOLIO_AVAILABLE:
            print("✓ Comprehensive Pyfolio tear sheets")
            print("✓ Professional performance metrics")
            print("✓ Risk analysis and visualizations")
        else:
        ...
        results = MinerviniEnhancedStrategy.backtest(
        ...
        print("\n🎉 ENHANCED MINERVINI STRATEGY COMPLETED!")
        print("✓ Basic CSV trade log exported")
        if PYFOLIO_AVAILABLE:
            print("✓ Pyfolio tear sheet generated")
            print("✓ Professional performance analysis available")
        print("✓ Enhanced reporting with risk metrics")
...
if __name__ == "__main__":
    # Check and install pyfolio if needed
    if not PYFOLIO_AVAILABLE:
        print("Installing pyfolio-reloaded...")
        try:
            import subprocess
            import sys
            subprocess.check_call([sys.executable, "-m", "pip", "install", "pyfolio-reloaded"])
            print("✓ Pyfolio-reloaded installed successfully!")
            print("Please restart and run again to use Pyfolio features.")
        except Exception as e:
            print(f"Could not install pyfolio-reloaded: {e}")
            print("Please install manually: pip install pyfolio-reloaded")
    
    print("Starting Enhanced Minervini Strategy with Pyfolio Analysis...")
    results = run_enhanced_minervini_backtest()