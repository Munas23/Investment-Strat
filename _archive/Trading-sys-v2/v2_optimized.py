...
# --- DATA FETCHING & INDICATOR CALCULATION (No changes here) ---
...
# --- ANALYSIS (UPDATED) ---
## --- FIX --- ##
# This function is updated to correctly analyze results from a `group_by=True` portfolio.
# --- (Keep all your existing code from the top down to here) ---
...
def analyze_results(portfolio, config):
    ...
    # Check for total number of trades first
    if portfolio.trades.count() == 0:
        print("\nNo trades were executed during the backtest period!")
        return stats # Return early if no trades
            
    try:
        ## --- FIX: Convert records to DataFrame manually --- ##
        trades_df = pd.DataFrame(portfolio.trades.records)
        tickers = portfolio.close.columns
        trades_df['Ticker'] = tickers[trades_df['col']]
        
        # Calculate total P&L for each stock
        stock_pnl = trades_df.groupby('Ticker')['pnl'].sum()

        if not stock_pnl.empty:
            print("\n=== Per-Stock PNL Contribution ===")
            print(f"Number of stocks with trades: {len(stock_pnl)}")
            print(f"Average PNL per stock: ${stock_pnl.mean():.2f}")
            print(f"Median PNL per stock: ${stock_pnl.median():.2f}")
            
            print("\n=== Top 5 Performing Stocks (by PNL) ===")
            top_5 = stock_pnl.nlargest(5)
            for ticker, pnl in top_5.items():
                print(f"{ticker}: ${pnl:,.2f}")
            
            print("\n=== Worst 5 Performing Stocks (by PNL) ===")
            bottom_5 = stock_pnl.nsmallest(5)
            for ticker, pnl in bottom_5.items():
                print(f"{ticker}: ${pnl:,.2f}")
        
    except Exception as e:
        print(f"\nError analyzing per-stock results: {e}")
        print("Could not generate detailed performance statistics.")
    
    return stats

def plot_best_stock(portfolio, indicators):
    """Plot the best performing stock (by PNL) with error handling."""
    if portfolio is None or portfolio.trades.count() == 0:
        print("\nNo trades to plot.")
        return

    try:
        # Get trades records and identify the best stock
        trades_df = pd.DataFrame(portfolio.trades.records)
        tickers = portfolio.close.columns
        trades_df['Ticker'] = tickers[trades_df['col']]
        ...
        if stock_pnl.empty:
            print("\nCould not determine the best performing stock.")
            return

        best_stock_symbol = stock_pnl.idxmax()
        print(f"\nPlotting trades for best performing stock (Total PNL: ${stock_pnl.max():,.2f}): {best_stock_symbol}")
        
        # Get the price data for the best stock and make it timezone-naive
        price_to_plot = indicators['price'][best_stock_symbol]
        if hasattr(price_to_plot.index, 'tz') and price_to_plot.index.tz is not None:
            price_to_plot = price_to_plot.tz_localize(None)
        
        # Create the base figure with the price line (no extra parameters)
        fig = price_to_plot.vbt.plot(title=f'Trades for {best_stock_symbol}')

        # Add the trades to the existing figure
        portfolio.trades.plot(column=best_stock_symbol, fig=fig)
...
def export_trades_to_csv(portfolio, filename="trades_export.csv"):
    """Exports all trades from the portfolio to a CSV file with detailed information."""
    if portfolio is None or portfolio.trades.count() == 0:
        print("No trades to export.")
        return

    print(f"\nExporting {portfolio.trades.count()} trades to {filename}...")

    try:
        # Get the main trade records as a DataFrame
        trades_df = pd.DataFrame(portfolio.trades.records)
        tickers = portfolio.close.columns
        date_index = portfolio.close.index
        
        # Check what columns are actually available
        print("Available trade record columns:", trades_df.columns.tolist())
        
        # Add basic information
        trades_df['Ticker'] = tickers[trades_df['col']]
        trades_df['Entry Date'] = date_index[trades_df['entry_idx']]
        trades_df['Exit Date'] = date_index[trades_df['exit_idx']]
        
        # Calculate total fees using the actual entry_fees and exit_fees columns
        trades_df['Fees'] = trades_df['entry_fees'] + trades_df['exit_fees']

        # Add calculated columns
        trades_df['Return (%)'] = trades_df['return'] * 100
        trades_df['Direction'] = np.where(trades_df['direction'] == 0, 'Long', 'Short')
        trades_df['Status'] = np.where(trades_df['status'] == 0, 'Open', 'Closed')
        trades_df['Duration (Days)'] = (trades_df['Exit Date'] - trades_df['Entry Date']).dt.days
        ...
        for old_col, new_col in column_mapping.items():
            if old_col in trades_df.columns:
                trades_df = trades_df.rename(columns={old_col: new_col})
        
        # Define columns to export (only include those that exist)
        base_columns = [
            'Trade ID', 'Ticker', 'Direction', 'Status',
            'Entry Date', 'Entry Price', 'Exit Date', 'Exit Price',
            'Duration (Days)', 'Position Size', 'Fees', 'Profit/Loss (PNL)',
            'Return (%)'
        ]
        
        # Add optional columns if they exist
        optional_columns = ['exit_reason', 'sl_price', 'tp_price']
        for col in optional_columns:
            if col in trades_df.columns:
                base_columns.append(col)
        
        # Filter to only include columns that actually exist in the DataFrame
        export_columns = [col for col in base_columns if col in trades_df.columns]
        
        # Export to CSV
        trades_df[export_columns].to_csv(filename, index=False, float_format='%.4f')
        print(f"Successfully exported trades to '{filename}'.")
...
def main():
    ...
    if portfolio is not None:
        ...
        csv_filename = f"trades_export_{timestamp}.csv"
        export_trades_to_csv(portfolio, filename=csv_filename)
