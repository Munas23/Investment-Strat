"""
5LC (5 Level Conviction) Strategy - ASX300 Implementation with Market Health Overlay
==================================================================================

Enhanced version of Minervini's methodology with:
1. Halved conviction percentages for more conservative position sizing
2. Market health overlay using VAS.AX (ASX300 ETF) as market indicator
3. Dynamic position sizing based on market conditions
4. Professional risk management and pyfolio analysis

Market Health Rules:
- HALVE positions when VAS.AX < 20MA AND < 50MA (weak bear markets)
- DOUBLE positions when VAS.AX > 20MA AND > 50MA AND > 200MA (strong bull markets)  
...
class Minervini5LCASX300Strategy(Strategy):
    """
    5LC ASX300 Strategy with Market Health Overlay
    
    Features:
    1. Halved conviction percentages (5%-20% vs 10%-40%)
    2. Market health overlay using VAS.AX moving averages
    3. Dynamic position sizing based on market regime
    4. Enhanced risk management for Australian market
    """
    ...
    def initialize(self):
        ...
        self.min_position_size = 0.20  # Original size
        ...
        self.log_message("=== 5LC ASX300 STRATEGY WITH MARKET HEALTH OVERLAY ===")
        self.log_message(f"Universe: ASX 300 ({len(self.asx300_symbols)} stocks)")
    ...
    def get_asx300_symbols(self):
        ...
        try:
            # ASX300 symbols with .AX suffix for Yahoo Finance
            asx300_sample = [
                # Big 4 Banks
                'CBA.AX', 'ANZ.AX', 'WBC.AX', 'NAB.AX',
                
                # Resources/Mining
                'BHP.AX', 'RIO.AX', 'FMG.AX', 'NCM.AX', 'WDS.AX', 'STO.AX',
                'WHC.AX', 'MIN.AX', 'IGO.AX', 'OZL.AX', 'NST.AX',
                
                # Technology
                'XRO.AX', 'WTC.AX', 'APT.AX', 'ZIP.AX', 'TNE.AX', 'MP1.AX',
                'NXT.AX', 'CPU.AX', 'KGN.AX', 'REA.AX',
                
                # Healthcare/Biotech
                'CSL.AX', 'COH.AX', 'RMD.AX', 'PME.AX', 'FPH.AX', 'SHL.AX',
                'VEA.AX', 'IMU.AX', 'MSB.AX', 'SDG.AX',
                
                # Retail/Consumer
                'WOW.AX', 'WES.AX', 'JBH.AX', 'HVN.AX', 'PMV.AX', 'SUL.AX',
                'BRG.AX', 'GUD.AX', 'BAP.AX', 'KMD.AX',
                
                # Infrastructure/Utilities
                'TCL.AX', 'TLS.AX', 'SYD.AX', 'AST.AX', 'APA.AX', 'SKI.AX',
                'VCX.AX', 'AGL.AX', 'ORG.AX', 'VEA.AX',
                
                # REITs
                'SCG.AX', 'GMG.AX', 'VCX.AX', 'CHC.AX', 'MGR.AX', 'URW.AX',
                'GPT.AX', 'LLC.AX', 'BWP.AX', 'SCP.AX',
                
                # Industrials
                'QAN.AX', 'VAH.AX', 'ALL.AX', 'QBE.AX', 'IAG.AX', 'MPL.AX',
                'BXB.AX', 'TNT.AX', 'ALX.AX', 'BKW.AX',
                
                # Growth/Emerging
                'A2M.AX', 'BAL.AX', 'PDN.AX', 'LTR.AX', 'AGE.AX', 'PLS.AX',
                'LKE.AX', 'NVX.AX', 'ZIP.AX', 'IOU.AX',
                
                # Additional diversification
                'MQG.AX', 'AMP.AX', 'SUN.AX', 'COL.AX', 'DOW.AX', 'IEL.AX',
                'EVN.AX', 'GOR.AX', 'WSA.AX', 'IPL.AX', 'ORE.AX', 'PNV.AX',
                'BEN.AX', 'CWN.AX', 'SWM.AX', 'ALU.AX', 'BOQ.AX', 'ING.AX'
            ]
            
            self.log_message(f"Successfully loaded {len(asx300_sample)} ASX300 symbols")
    ...
    def update_fundamental_leaders(self):
        ...
        if (self.last_screening_date is None or 
            ...
            sample_size = min(50, len(self.asx300_symbols))  # Start with 50 ASX stocks
    ...
    def calculate_position_size(self, symbol, price, conviction_level):
        ...
        try:
            ...
            base_position_pct = {
                1: 0.20,   # 10% - minimal conviction
                2: 0.25,  # 12.5% - low conviction  
                3: 0.30,   # 15% - standard conviction
                4: 0.35,  # 17.5% - high conviction
                5: 0.40    # 20% - maximum conviction
    ...
    def check_championship_exits(self):
        ...
        for position in positions:
            try:
                ...
                # 4. Time Exit (12 months max for ASX)
    ...
    def scan_for_championship_entries(self):
        ...
        for symbol in candidates[:15]:  # Check top 15 ASX leaders
            try:
    ...
    def on_strategy_end(self):
        ...
        self.log_message("=== 5LC ASX300 STRATEGY COMPLETED ===")
        
        # Save basic CSV
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        if self.trades_log:
            trades_df = pd.DataFrame(self.trades_log)
            csv_filename = f"5lc_asx300_trades_{timestamp}.csv"
        ...
        self.log_message("=" * 80)
        self.log_message("5LC ASX300 STRATEGY PERFORMANCE & DEBUGGING")
        self.log_message("=" * 80)
        ...
        self.log_message("=" * 80)
        self.log_message("5LC ASX300 strategy completed - Check CSV and pyfolio reports")
    ...
    def generate_pyfolio_analysis(self, timestamp):
        ...
        returns_series = pd.Series(
            ...
            index=self.dates[1:],
            name='5lc_asx300_strategy_returns'
        )
        
        benchmark_series = pd.Series(
            self.benchmark_returns[1:],
            index=self.dates[1:],
            name='vas_benchmark_returns'
        )
        ...
        try:
            ...
            plt.savefig(f"{output_dir}/5lc_asx300_returns_tearsheet_{timestamp}.png", 
                       dpi=300, bbox_inches='tight')
            plt.close('all')
            self.log_message(f"✓ Returns tear sheet saved: 5lc_asx300_returns_tearsheet_{timestamp}.png")
            ...
            plt.savefig(f"{output_dir}/5lc_asx300_simple_tearsheet_{timestamp}.png", 
                       dpi=300, bbox_inches='tight')
            plt.close('all')
            self.log_message(f"✓ Simple tear sheet saved: 5lc_asx300_simple_tearsheet_{timestamp}.png")
            
            # 3. Generate and save key metrics to text file
            metrics_filename = f"{output_dir}/5lc_asx300_performance_metrics_{timestamp}.txt"
            ...
            self.log_message(f"  Benchmark: VAS.AX (ASX300 ETF)")
    ...
    def save_performance_metrics(self, returns, benchmark, filename):
        ...
        try:
            with open(filename, 'w') as f:
                f.write("5LC ASX300 STRATEGY PERFORMANCE METRICS\n")
                f.write("=" * 50 + "\n\n")
                ...
                f.write(f"Benchmark Return (VAS.AX): {benchmark_return:.2%}\n")
                ...
                f.write(f"- Market health overlay using VAS.AX moving averages\n")
                f.write(f"- Dynamic position sizing based on market regime\n")
                f.write(f"- Enhanced risk management for Australian market\n\n")
    ...
    def create_basic_performance_report(self, returns, benchmark, timestamp):
        ...
        try:
            ...
            axes[0,0].plot(cum_returns.index, cum_returns, label='5LC ASX300 Strategy', linewidth=2)
            axes[0,0].plot(cum_benchmark.index, cum_benchmark, label='VAS Benchmark', linewidth=2)
            ...
            # Save plot
            plot_filename = f"pyfolio_reports/5lc_asx300_performance_{timestamp}.png"
...
def run_5lc_asx300_backtest():
    """Run complete 5LC ASX300 strategy backtest"""
    try:
        print("=" * 80)
        print("5LC (5 Level Conviction) STRATEGY - ASX300 WITH MARKET HEALTH OVERLAY")
        print("=" * 80)
        print("Features:")
        ...
        print("  - WEAK (VAS < 20MA & 50MA): 0.5x positions")
        print("  - STRONG (VAS > 20MA & 50MA & 200MA): 2.0x positions") 
        print("  - NORMAL (other conditions): 1.0x positions")
        print("=" * 80)
        
        results = Minervini5LCASX300Strategy.backtest(
...
if __name__ == "__main__":
    print("Starting 5LC ASX300 Strategy with Market Health Overlay...")
