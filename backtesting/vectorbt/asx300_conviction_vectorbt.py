"""
ASX300 5-LEVEL CONVICTION STRATEGY - VECTORBT IMPLEMENTATION
===========================================================

Dedicated ASX300 implementation using VectorBT for independent validation
of our proven 5-Level Conviction trading strategy results.

This implementation mirrors our ASX300 lumibot strategy exactly but uses
...
class ASX300ConvictionVectorBT:
    """
    ASX300 5-Level Conviction Strategy - VectorBT Implementation
    
    Independent validation of our systematic approach using vectorized
    calculations instead of event-driven simulation for the Australian market.
    """
    ...
    def __init__(self, initial_cash=100000):
        ...
        # ASX300 specific parameters
        self.min_price = 2.00  # AUD minimum
        self.min_volume = 100000  # Lower threshold for ASX
        self.benchmark = "VAS.AX"  # ASX300 ETF
        self.currency = "AUD"
        ...
        print("=" * 65)
        print("ASX300 5-LEVEL CONVICTION STRATEGY - VECTORBT VALIDATION")
        print("=" * 65)
        print("Independent implementation for ASX300 results verification")
        print(f"Initial Capital: AUD {initial_cash:,}")
        print(f"Market: ASX300 (Australian Securities Exchange)")
        print(f"Benchmark: {self.benchmark}")
        print(f"Currency: {self.currency}")
        print(f"Min Price: AUD {self.min_price}")
    ...
    def get_asx300_symbols(self):
        ...
        print("\nGenerating ASX300 symbol universe...")
        
        # Top ASX300 stocks by market cap with .AX suffix
        asx300_symbols = [
            # Top 50 ASX stocks
            'CBA.AX', 'BHP.AX', 'CSL.AX', 'WBC.AX', 'ANZ.AX',
            'NAB.AX', 'WES.AX', 'MQG.AX', 'TCL.AX', 'WOW.AX',
            'FMG.AX', 'TLS.AX', 'RIO.AX', 'COL.AX', 'GMG.AX',
            'QBE.AX', 'STO.AX', 'WDS.AX', 'REA.AX', 'JHX.AX',
            'ALL.AX', 'XRO.AX', 'IAG.AX', 'COH.AX', 'WTC.AX',
            'CPU.AX', 'RMD.AX', 'PME.AX', 'NCM.AX', 'CAR.AX',
            'JBH.AX', 'S32.AX', 'ASX.AX', 'MIN.AX', 'QAN.AX',
            'AMP.AX', 'WOR.AX', 'SGP.AX', 'ALU.AX', 'CWN.AX',
            'IEL.AX', 'SUL.AX', 'BXB.AX', 'AZJ.AX', 'ORI.AX',
            'TWE.AX', 'LLC.AX', 'SHL.AX', 'BEN.AX', 'EVN.AX',
            
            # Additional liquid ASX stocks
            'APT.AX', 'ZIP.AX', 'PLS.AX', 'LYC.AX', 'PNV.AX',
            'ILU.AX', 'SYD.AX', 'SCG.AX', 'ORG.AX', 'CHC.AX',
            'IGO.AX', 'ALD.AX', 'TPG.AX', 'VCX.AX', 'BOQ.AX',
            'BSL.AX', 'NEC.AX', 'AGL.AX', 'ORG.AX', 'DXS.AX',
            'ALX.AX', 'WHC.AX', 'IPL.AX', 'SUN.AX', 'CQR.AX',
            'NST.AX', 'NXT.AX', 'IFM.AX', 'PDN.AX', 'APA.AX',
            'GPT.AX', 'MGR.AX', 'SKI.AX', 'MPL.AX', 'TNE.AX',
            'NEA.AX', 'URW.AX', 'DOM.AX', 'BLD.AX', 'LOV.AX',
            'CLW.AX', 'HSN.AX', 'CNU.AX', 'SEK.AX', 'ING.AX',
            'NUF.AX', 'SDF.AX', 'PBH.AX', 'HVN.AX', 'JHG.AX'
        ]
        
        print(f"ASX300 Universe: {len(asx300_symbols)} symbols")
        print("Sample symbols: " + ", ".join(asx300_symbols[:10]))
        
        return asx300_symbols
    
    def get_market_data(self, symbols, start_date, end_date):
        """Download ASX300 market data"""
        
        print(f"\nDownloading ASX300 market data...")
        ...
        try:
            ...
            print(f"Downloaded ASX300 data: {len(data)} trading days")
            print(f"Data columns per symbol: {data.columns.get_level_values(1).unique().tolist()}")
            
            return data
            
        except Exception as e:
            print(f"Error downloading ASX300 data: {e}")
            return None
    
    def calculate_fundamental_score(self, symbol):
        """
        Calculate fundamental quality score (simplified for VectorBT)
        Returns consistent score for backtesting purposes
        """
        
        # Simplified fundamental scoring for validation
        # In production, this would connect to fundamental data provider
        
        # Assign quality scores based on ASX300 market knowledge
        # Top tier companies get higher scores
        top_tier = ['CBA.AX', 'BHP.AX', 'CSL.AX', 'WBC.AX', 'ANZ.AX', 
                   'NAB.AX', 'WES.AX', 'MQG.AX', 'WOW.AX', 'RIO.AX']
        
        high_quality = ['FMG.AX', 'TLS.AX', 'COL.AX', 'QBE.AX', 'STO.AX',
                       'REA.AX', 'ALL.AX', 'XRO.AX', 'COH.AX', 'RMD.AX']
        
        if symbol in top_tier:
            return np.random.uniform(75, 95)  # High quality scores
        elif symbol in high_quality:
            return np.random.uniform(65, 85)  # Good quality scores
        else:
            return np.random.uniform(45, 75)  # Variable quality scores
    
    def apply_market_filters(self, data, symbols):
        """Apply ASX300 specific market filters"""
        
        print(f"\nApplying ASX300 market filters...")
        ...
        print(f"  Price filter (< AUD {self.min_price}): {filter_results['price_filter']} removed")
    ...
    def calculate_technical_indicators(self, data, symbol):
        """Calculate technical indicators for ASX300 symbols"""
    ...
    def calculate_trend_strength(self, indicators):
        """Calculate trend strength score (0-100) - ASX optimized"""
        ...
        # Momentum (20 points) - adjusted for ASX volatility
        score += (momentum_20d > 3) * 10  # Lower threshold for ASX
        score += (momentum_50d > 8) * 10   # Lower threshold for ASX
        
        # Near highs (20 points)
        distance_from_high = (close / high_50 - 1) * 100
        score += (distance_from_high > -15) * 20  # More lenient for ASX
        
        return score
    
    def generate_conviction_signals(self, indicators):
        """Generate ASX300 conviction-based entry signals (0-5)"""
        ...
        # Factor 1: Breakout power (0-25 points) - ASX adjusted
        breakout_20 = close > (high_20 * 1.008)  # Slightly lower for ASX
        breakout_50 = close > (high_50 * 1.015)  # Slightly lower for ASX
        conviction += breakout_20 * 15
        conviction += (breakout_20 & breakout_50) * 10
        
        # Factor 2: Volume confirmation (0-30 points) - ASX adjusted
        volume_surge = volume / volume_avg_20
        conviction += (volume_surge > 1.8) * 30   # Lower threshold for ASX
        conviction += ((volume_surge > 1.4) & (volume_surge <= 1.8)) * 20
        conviction += ((volume_surge > 1.1) & (volume_surge <= 1.4)) * 10
        
        # Factor 3: Momentum alignment (0-25 points) - ASX adjusted
        conviction += (momentum_5d > 0.8) * 5     # Lower for ASX volatility
        conviction += (momentum_20d > 3) * 10     # Lower for ASX volatility  
        conviction += (momentum_50d > 8) * 10     # Lower for ASX volatility
        ...
        # Convert to conviction levels (ASX optimized thresholds)
        conviction_levels = pd.Series(0, index=close.index)
        conviction_levels[conviction >= 80] = 5   # Maximum conviction
        conviction_levels[(conviction >= 65) & (conviction < 80)] = 4  # High conviction
        conviction_levels[(conviction >= 50) & (conviction < 65)] = 3  # Standard conviction
        conviction_levels[(conviction >= 35) & (conviction < 50)] = 2  # Low conviction
        conviction_levels[(conviction >= 20) & (conviction < 35)] = 1  # Minimal conviction
        
        return conviction_levels, conviction, trend_strength
    
    def run_asx300_backtest(self, start_date='2015-01-01', end_date='2024-01-01'):
        """Run complete ASX300 backtest with detailed analysis"""
        
        print(f"\nSTARTING ASX300 VECTORBT VALIDATION BACKTEST")
        print(f"Market: Australian Securities Exchange (ASX300)")
        print(f"Currency: {self.currency}")
        print(f"Period: {start_date} to {end_date}")
        ...
        print(f"\nRunning ASX300 strategy simulation...")
        
        results = {}
        all_trades = []
        
        for symbol in filtered_symbols[:50]:  # Limit for initial testing
        ...
        portfolio_performance = self.calculate_asx300_performance(results, all_trades, data)
        
        return results, portfolio_performance
    
    def calculate_asx300_performance(self, results, trades, data):
        """Calculate comprehensive ASX300 portfolio performance metrics"""
        
        print(f"\nCALCULATING ASX300 PORTFOLIO PERFORMANCE")
        ...
        print(f"Market: ASX300 ({self.currency})")
    ...
    def generate_detailed_trades(self, symbol, indicators, conviction_levels, conviction_scores, trend_strength):
        """Generate detailed trade records with entry/exit reasons"""
        ...
        for date in entry_signals[entry_signals].index:
            ...
            # Determine entry reasons
            entry_reasons = []
            
            # Breakout analysis
            if close.loc[date] > (high_20.loc[date] * 1.008):
                entry_reasons.append(f"20-day breakout (+{((close.loc[date]/high_20.loc[date])-1)*100:.1f}%)")
            if close.loc[date] > (high_50.loc[date] * 1.015):
            ...
            # Momentum analysis
            if not pd.isna(momentum_5d.loc[date]) and momentum_5d.loc[date] > 0.8:
                entry_reasons.append(f"5d momentum (+{momentum_5d.loc[date]:.1f}%)")
            if not pd.isna(momentum_20d.loc[date]) and momentum_20d.loc[date] > 3:
                entry_reasons.append(f"20d momentum (+{momentum_20d.loc[date]:.1f}%)")
            if not pd.isna(momentum_50d.loc[date]) and momentum_50d.loc[date] > 8:
    ...
    def simulate_portfolio_returns(self, trades, data):
        """Simulate realistic portfolio returns with detailed metrics"""
        ...
        for symbol, symbol_trade_list in symbol_trades.items():
            ...
            for trade in symbol_trade_list:
                ...
                position_value = trade['position_value_aud']
    ...
    def export_asx300_results(self, results, performance):
        """Export comprehensive ASX300 results for analysis"""
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Export performance summary
        performance_df = pd.DataFrame([performance])
        performance_filename = f"vectorbt_asx300_performance_{timestamp}.csv"
        ...
        if all_detailed_trades:
            ...
            detailed_trades_filename = f"vectorbt_asx300_detailed_trades_{timestamp}.csv"
        ...
        trades_filename = f"vectorbt_asx300_trades_{timestamp}.csv"
        trades_df.to_csv(trades_filename, index=False)
        
        print(f"\nASX300 VECTORBT RESULTS EXPORTED:")
...
def main():
    """Main execution for ASX300 VectorBT testing"""
    
    print("=" * 70)
    print("ASX300 5-LEVEL CONVICTION STRATEGY - VECTORBT VALIDATION")
    print("=" * 70)
    print("Independent ASX300 implementation for results verification")
    print("Comparing against our existing ASX300 lumibot results")
    print()
    
    # Initialize ASX300 strategy
    strategy = ASX300ConvictionVectorBT()
    
    # Run backtest
    results, performance = strategy.run_asx300_backtest('2020-01-01', '2024-01-01')
    
    if results and performance:
        # Export results
        perf_file, trades_file = strategy.export_asx300_results(results, performance)
        
        print(f"\nSUCCESS: ASX300 VECTORBT VALIDATION COMPLETE!")
        print("=" * 50)
        print("Results ready for comparison with ASX300 lumibot strategy")
        print(f"Performance file: {perf_file}")
        print(f"Trades file: {trades_file}")
        print("\nNext: Run S&P500 VectorBT validation")
    else:
        print("ERROR: ASX300 VectorBT test failed - check implementation")
